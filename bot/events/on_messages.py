...
from bot.utils.ai import gentext
...
class MessageHandler(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if not collection.find_one({"id": message.channel.id}):
            return

        async with message.channel.typing():
            if message.author.id not in self.bot.history:
                self.bot.history[message.author.id] = []
