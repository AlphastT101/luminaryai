...
def image():
    ...
    if not user_id.startswith('owner'):
        response = requests.get(f"https://discord.com/api/v10/guilds/{guild_id}/roles/1279261339574861895", headers=headers)
        member_response = requests.get(f"https://discord.com/api/v10/guilds/{guild_id}/members/{user_id}", headers=headers)
    ...
    if engine == "poli":
        ...
        engine_id = "1" if engine == "dalle3" else "2" if engine == "sdxl-turbo" else "3"
